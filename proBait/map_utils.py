...
def run_minimap2(reference, map_fasta, output_file, threads, mode):
	...
	if mode == 'short':
		if len(map_fasta) > 1:
			reads_str = ' '.join(map_fasta)
		else:
			reads_str = map_fasta[0]
		...
		minimap_args = ['minimap2 -I 100M --cs -cx sr --secondary=yes -t {3} {0} {1} > '
						'{2}'.format(reference, reads_str, output_file, threads)]
	elif mode == 'long':
		# To align long reads
		minimap_args = ['minimap2 -I 100M --cs -cx lr:hq --secondary=yes -t {3} {0} {1} > '
						'{2}'.format(reference, map_fasta, output_file, threads)]
	elif mode == 'contigs':
		# To align genomic sequences in FASTA format
		minimap_args = ['minimap2 -I 100M --cs -cx asm10 --secondary=yes -t {3} {0} {1} > '
						'{2}'.format(reference, map_fasta, output_file, threads)]
